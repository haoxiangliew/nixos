...
Most of the [[https://git.sr.ht/~haoxiangliew/nixos][dotfiles]] found in other configurations are managed by home-manager in this nixos configuration, except for emacs

Configuration is daily-driven and maintained on nixos-unstable

** Installation
1. Boot into the latest [[https://channels.nixos.org/nixos-unstable/latest-nixos-minimal-x86_64-linux.iso][nixos-unstable]]
2. To simply configure the wifi with wpa-supplicant
#+BEGIN_SRC sh
wpa_supplicant -B -i interface -c <(wpa_passphrase 'SSID' 'key')
#+END_SRC
3. Set partitions (we use parted here)
#+BEGIN_SRC sh
...
reboot
#+END_SRC
8. Login as ~root~, ~git clone~ this repository, ~passwd~ your username, and ~nixos-rebuild switch~
